...
$dbname = "tourofcrete";
$tocURL = '<a href = "http://localhost/index.php?stage=';
...
//DEBUG_MSG("Βρισκόμαστε στο " . $current_stage . " stage που έχει κωδικό στη βάση: " . $current_stage_id);
/*
****************************************************************************************************************************************/
...
//DEBUG_MSG("Έχει επιλεγεί το " . $selected_stage . " stage που έχει κωδικό στη βάση: " . $selected_stage_id);
/*
****************************************************************************************************************************************/
...
//
// Στην έκδοση αυτή έχουμε δημιουργήσει ένα view, το tv_events, το οποίο κάνει join με
// τον πίνακα tc_positions για να πάρουμε από κει το fixtime. Το fixtime είναι για την περίπτωσή μας
// πιο "σωστό" από το tc_events.servertime στο οποίο βασίζονταν ο προηγούμενος αλγόριθμος.
// Βλέπε https://www.traccar.org/forums/topic/geofence-enterext-time-how-is-calculated/

	$events_sql = "SELECT type, fixtime, deviceid, ascent_id, stage_id, race_id, locked
					FROM tourofcrete.tv_events
					WHERE locked = 0";

	$events_result = $conn->query($events_sql);
	if ($events_result->num_rows > 0) {
	$i=0;
	//DEBUG_MSG("Πλήθος=" . $events_result->num_rows);
	while($row = $events_result->fetch_assoc()) {
		$key = $row["deviceid"] . "#" . $row["ascent_id"] . "#" . $row["stage_id"] . "#" . $row["race_id"];
		$i=$i+1;
		if ($row["type"] == "geofenceEnter") {
			// Καταχωρούμε το πρώτο geofenceEnter που συναντάμε
			...
			$timetable_rec["time_in"] = $row["fixtime"];
			$timetable_rec["update"] = "no";
			if (!array_key_exists($key, $timetable)) {
				$timetable[$key] = $timetable_rec;
			}
		} elseif ($row["type"] == "geofenceExit") {
			// Καταχωρούμε το πρώτο geofenceExit που συναντάμε
			if (array_key_exists($key, $timetable)) {
				if ($timetable[$key]["update"] == "no") {
					$timetable[$key]["time_out"] = $row["fixtime"];
...
<p style="color:red">
* Terry, you have to kill Brad to win a yellow jersey. I do not see any other way :-)))
</p>
<p style="color:red">
** DNF changed to DNS. As Terry said, it suits better because the cyclists never started for that stage :-))))
</p>
!-->
<div id="wrapper">
<div id="stage_links">
<?php
	...
	for ($a = 1; $a <= $current_stage; $a++) {
		if ($a == $selected_stage) {
			...
			$html_str = $html_str . $tocURL . $a .'" class="stage_links"> STAGE ' . $a . '</a> | ';
		}
	}
...
<?php
echo '<br><br>';
echo 'Click <a href="stage-' . $selected_stage . '.htm">here</a> to see a ranking list of all the participants.'
?>
</div>
</body>
</html>
